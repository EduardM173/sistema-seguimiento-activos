...
import { SmartGalery } from '../components/common/SmartGalery';
...
import { IconInfo, IconEdit, IconGrid, IconClock, IconLayoutList } from '@/components/common/Icon';
...
export default function AssetsPage() {
  ...
  // Extract prefill data when the create-asset modal is opened via a deeplink
  // generated by the AI assistant (params like prefill_nombre, prefill_marca, etc.)
  const createAssetPrefill = useMemo(() => {
    if (!showCreateAssetModal) return undefined;
    // Include both id-based params (from AI wizard) and name-based fallbacks
    const fields = [
      'nombre', 'marca', 'modelo', 'numeroSerie', 'descripcion',
    ...
    for (const key of fields) {
      const val = searchParams.get(`prefill_${key}`);
      if (val) result[key] = val;
    }
    return Object.keys(result).length > 0 ? result : undefined;
  }, [showCreateAssetModal, searchParams]);
  ...
  if (!bajaMotivo.trim()) {
    notify.warning('Motivo requerido', 'El motivo de baja es obligatorio.');
    return;
  }
